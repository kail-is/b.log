...
> 지금 맡은 서비스 환경에서는 MQ를 붙일 수 없지만, 조건이 같다면 분명 MQ를 써야 할 장면들이 많다. 특정 시기 때는 Kafka 쓰면 된다, 라고 인지하고 있음을 말하는 대신, 직접 메시지 큐를 만들어 보며 감을 다듬고 싶었다. Mini Kafka는 제약을 뛰어넘어 어떻게든 MQ를 설계해 보려는 시도였다.
